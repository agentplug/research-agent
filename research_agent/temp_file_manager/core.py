...
from ..utils.utils import format_response, get_current_timestamp, safe_json_dumps, safe_json_loads
...
class TempFileManager:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.base_directory = Path(self.temp_config.get('base_directory', '/tmp/research_agent'))
        ...
        self.organization = self.temp_config.get('organization', {
        ...
        try:
            if not self.enabled:
                return format_response(
                    success=True,
                    data={'initialized': False, 'reason': 'Temp file management disabled'},
            ...
            return self.error_handler.handle_error(
                e,
                {'session_id': session_id},
                f"Error initializing session: {str(e)}"
            )
    ...
    def store_research_data(
        ...
        try:
            ...
            if isinstance(data, str):
                ...
                data_size_mb = len(safe_json_dumps(data).encode('utf-8')) / (1024 * 1024)
            else:
    ...
    def get_cached_data(self, key: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return self.error_handler.handle_error(
                e,
                {'key': key},
                f"Error getting cached data: {str(e)}"
            )
    ...
    def list_session_files(self, data_type: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
                ...
                for dir_name, dir_path in self.organization.items():
                    ...
                    if full_path.exists():
                        files[dir_name] = [
                            ...
                            for f in full_path.iterdir() if f.is_file()
                        ]
            ...
            return self.error_handler.handle_error(
                e,
                {'data_type': data_type},
                f"Error listing session files: {str(e)}"
            )
    ...
    def cleanup_session(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"🧹 Cleaned up session: {self.current_session_id} ({session_size} bytes)")
    ...
    def cleanup_old_sessions(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"🧹 Cleaned up {len(cleaned_sessions)} old sessions ({total_size_freed} bytes)")
            ...
            return self.error_handler.handle_error(
                e,
                {},
                f"Error cleaning up old sessions: {str(e)}"
            )
