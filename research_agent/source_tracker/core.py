...
class SourceTracker:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.reliability_threshold = self.source_config.get(
        ...
        self.round_sources: Dict[
            int, List[str]
        ] = {}  # round -> list of normalized_urls
    ...
    def add_source(
        ...
        try:
            ...
            if not url or not isinstance(url, str):
                return format_response(success=False, message="Invalid URL provided")

            # Normalize URL
            normalized_url = self._normalize_url(url)
            if not normalized_url:
                return format_response(success=False, message="Failed to normalize URL")
            ...
            reliability_score = self._calculate_reliability_score(
                normalized_url, title, description
            )
            ...
            logger.info(
                f"📚 Added source: {normalized_url} (reliability: {reliability_score:.2f})"
            )
    ...
    def add_sources_batch(
        self, sources: List[Dict[str, Any]], round_number: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        """
        try:
            results: Dict[str, List[Any]] = {"added": [], "filtered": [], "errors": []}

            for source in sources:
                url = source.get("url")
                if not url:
                    results["errors"].append({"source": source, "error": "Missing URL"})
                    continue
                ...
                if result["success"]:
                    ...
                    results["errors"].append(
                        {"url": source.get("url"), "error": result["message"]}
                    )
    ...
    def get_all_sources(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return self.error_handler.handle_error(
                e, {}, f"Error getting all sources: {str(e)}"
            )
    ...
    def get_sources_by_domain(self, domain: str) -> Dict[str, Any]:
        ...
        try:
            domain_sources = [
                source for source in self.sources.values() if source["domain"] == domain
            ]
            ...
            return self.error_handler.handle_error(
                e, {"domain": domain}, f"Error getting sources by domain: {str(e)}"
            )

    def get_high_reliability_sources(
    ...
    def _calculate_reliability_score(
        ...
        if title:
            ...
            if any(
                word in title_lower
                for word in ["study", "research", "analysis", "report"]
            ):
                score += 0.1

            # Negative indicators
            if any(
                word in title_lower for word in ["advertisement", "sponsored", "promo"]
            ):
                score -= 0.2
        ...
        if description:
            ...
            if any(
                word in desc_lower
                for word in ["peer-reviewed", "academic", "scholarly"]
            ):
                score += 0.15
    ...
    def clear_session(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return self.error_handler.handle_error(
                e, {}, f"Error clearing session: {str(e)}"
            )
